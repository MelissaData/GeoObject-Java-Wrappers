...
- Melissa GeoCoder Object for 2024-Q1
...
Import the `com` folder, and `mdGeoJavaWrapper.dll` (Windows) or `libmdGeoJavaWrapper.so` (Linux) to your project, and include the following line in your code to begin using the wrapper.
...
#### Configure Scripts
If you want to generate `mdGeoJavaWrapper.dll`, you will execute the `BuildGeoJavaWrapper.ps1` script. This may require you to configure some path variables in `Build.ps1` and `BuildGeoJavaWrapper.ps1`.
...
It is important to note that you must be able to initialize the Visual Studio Developer Command Prompt environment for `x86_x64` to generate `mdGeoJavaWrapper.dll`.
...
#### Configure Script
If you want to generate `libmdGeoJavaWrapper.so`, you will execute the `libmdGeoJavaWrapper.so` script. This may require you to configure some path variables in `libmdGeoJavaWrapper.so`.

libmdGeoJavaWrapper.so
- Check the `JavaIncludeDirectory1` variable and configure the path to the `include` folder if needed
- Check the `JavaIncludeDirectory2` variable and configure the path to the `win32` folder if needed
