...
Config_FileName4="mdGeoJavaWrapper.cpp"
...
DownloadFiles() 
{
  ...
  if [ ! -f "$ProjectPath/mdGeo_JavaCode.zip" ];
  then
      printf "mdGeo_JavaCode.zip not found.\n"
      ...
      then
          unzip "$ProjectPath/mdGeo_JavaCode.zip" -d "$ProjectPath"
      else
...
# Compile SO
gcc -fpic -c -I$ProjectPath -I$JavaIncludeDirectory1 -I$JavaIncludeDirectory2 mdGeoJavaWrapper.cpp
gcc -g -shared mdGeoJavaWrapper.o -o libmdGeoJavaWrapper.so -L$ProjectPath -lmdGeo -pthread
...
jar cvf mdGeo.jar com/melissadata/*.class &> /dev/null
...
then
  script_dir="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
  file_path="$script_dir/mdGeo.jar"

  printf "\nmdGeo.jar has been successfully generated at:\n$file_path\n"
else
  printf "\nError: was not able to create mdGeo.jar\n"
fi
